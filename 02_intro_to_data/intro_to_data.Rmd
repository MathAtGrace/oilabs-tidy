...
1.  Using what we learned from section 1.2 from the textbook, what kind of variable is the variable `dep_delay`?  What kind of variable is `carrier`? (Remember to use complete sentences in your lab reports)
...
2.  Put each of the three histograms into your lab report. How do they compare? Are features revealed in one that are obscured in another?
...
1.  Copy the code from the code block above into a code block in your lab report.  Write another line that will tell you how many flights are in this data frame.  (Hint: it's a command that is present somewhere above)

2.  Describe the distribution of the **arrival** delays of these flights by including a histogram, using descriptive words we learned in section 2.1, and appropriate summary statistics.
...
1.  Consider the code below.  Tell me what the new variables `air_time_hr` and `ave_speed` describe, and what units they are measured in.
```{r 8}
nycflights <- nycflights %>%
