...
* Start a lab from a template provided by our textbook.
* Use the basic commands of R
* View data sets in RStudio
* View help files in RStudio
* Perform basic data manipulations with the `tidyverse` package.
* Create basic data visualizations with `ggplot` package.


## The RStudio Interface

Before we begin, you should have installed two programs: First `R` and then `RStudio`.  R is like the engine, and RStudio is like the car.  Just like how you shouldn't mess with your engine if you haven't been trained on one, you shouldn't mess with R.  For these labs, we'll be using RStudio.
...
The panel on the lower left is called the *console.*  Think of it like a fancy calculator.  You can use it to compute and experiment, but just like a caclulator, it's hard to show your work if its the only thing you use.

Every time you launch RStudio, it will have the same text at the top of the console telling you the version of R that you're running. Below that information is the *prompt*.
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/lNWVQ2oxNho" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
To begin, in RStudio, go to New File -\> R Markdown... Then, choose "From Template" and then choose `Lab Report for OpenIntro Statistics Lab 1` from the list of templates.
...
For these labs, use the console for playing around, and use the code chunk in your lab report for showing your work for your final answer.
...
Within the code chunk there are two ways to execute a line of `R` code: (1) place your cursor on the line on code and press `Ctrl-Enter` (or `Cmd-Enter` for Mac) at the same time, or (2) place your cursor on the line and press the "Run" button in the upper right hand corner of the R Markdown file. Alternatively, if you wanted to run all of the `R` code in a given code chunk, you can click on the "Play" button in the upper right hand corner of the code chunk (green sideways triangle).
...
To get started, let's take a peek at the data. Copy and paste the code below into your code chunk and run it.

```{r load-arbuthnot-data}
data("arbuthnot")
```

You should see that the *Environment* tab in the upper right hand corner of the RStudio window now lists a data set called `arbuthnot` that has 82 observations on 3 variables.  There are three ways to view this data.  The simplist is to copy/paste the code below into your console or your code chunk and run it.

```{r simple-view-arbuthnot-data}
arbuthnot
```

The *Environment* tab (in the upper right pane) lists the objects in your environment. Clicking on the name `arbuthnot` will open up a *Data Viewer* tab next to your R Markdown file, which provides an alternative display of the data set. This display should feel similar to viewing data in Excel, where you are able to scroll through the dataset to inspect it. However, unlike Excel, you **will not** be able to edit the data in this tab. Once you are done viewing the data, You can close this tab by clicking on the `x` in the upper left hand corner.

A third way does exactly the same thing: run this code.

```{r view-abrbuthnot-data, eval=FALSE}
View(arbuthnot)
```
...
You can see the dimensions of this data frame as well as the names of the variables and the first few observations by inserting the name of the dataset into the `glimpse()` function, as seen below.  Copy/paste this into your console:
...
1.  What command would you use to extract just the counts of girls baptized?  In these lab reports, also use complete sentences and the text -> code -> text -> code pattern described in the video above.

Notice that the way `R` has printed these data is different. `R` prints each column as a *vector*; similar to the vectors you have seen in mathematics courses, vectors represent a list of numbers. `R` has added numbers displayed in [brackets] along the left side of the printout to indicate each entry's location within the vector.
...
In this code, we use the `ggplot()` function to build a plot.  If you run this code in your console, then a plot shows up in the lower right section. If you run this in a code chunk, a plot will appear below the code chunk. Try both.
...
1.  Is there an apparent trend in the number of girls baptized over the years? How would you describe it? (To ensure that your lab report is comprehensive, be sure to include the code needed to make the plot as well as your written interpretation *using full sentences*.)
...
Now let's view our `arbuthnot` data frame with the new column we created:
```{r}
View(arbuthnot)
```
...
4.  Create a new variable for the `present` data called `total` that adds up boys and girls just like we did with arbuthnot's.  For this question, you only need to present the code you used as an code chunk.

5.  In what year did we see the most total number of births in the U.S.? *Hint:* Sort your dataset in descending order based on the `total` column. You can do this interactively in the data viewer by clicking on the arrows next to the variable names. To include the sorted result in your report you will need to use two new functions. First we use `arrange()` to sorting the variable. Then we can arrange the data in a descending order with another function, `desc()`, for descending order. The sample code is provided below.
...
### Bonus questions for understanding R and RStudio

5.  Use complete sentence(s) to tell me when you need to intall packages and when you need to load packages with the `library` function.

6.  Use complete sentence(s) to tell me the three ways to view data.
