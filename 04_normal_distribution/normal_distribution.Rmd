...
1.  For both the `mcdonalds` and `dairy_queen` data sets, plot the variable `cal_from_fat`, and describe the center, shape, and spread of each.  Make sure to use numbers when applicable in your descriptions.  If your memory of how to plot data is rusty, go back to labs 1 and 2.
...
2.  Put this plot into your lab report, and describe at least one similarity and one difference between the histogram of actual data and the theoretical normal distribution.
...
We can answer this by simulating data from a normal distribution using `rnorm` and putting this new simulated variable into our dataframe.
...
The first argument of `rnorm` indicates how many numbers you'd like to generate, which we specify to be the same number of menu items in the `dairy_queen` data set using the `nrow()` function.
...
3.  Make a normal probability plot of `sim_cal_fat`. Do all of the points fall on the line? How does this plot compare to the probability plot for the real data?
...
4.  Create the plot in your lab report with the code given above. Does the normal probability plot for the calories from fat look similar to the plots created for the simulated data?  That is, do the plots provide evidence that the distribution of calories from fat of the menu items are nearly normal?

5.  Using the same technique, create the plot and determine whether or not the calories from McDonald's menu appear to come from a normal distribution.
...
7.  Now let's consider some of the other variables in the dataset.
    Out of all the different restaurants, which two or three or four distribution are the closest to normal for cholesterol?  The code below gives you the list of restaurants in this data.
