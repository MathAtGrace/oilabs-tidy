...
For example, here is a Chi-squared test for independence between how many hours students slept on school nights vs the number of hours they watched TV on school nights.  First let's give the categories a proper order
...
We can run a chi-squared test on this data by creating a two-way table and then passing it to the funciton `chisq.test()`.
...
When you ran the code chunk above, R did a lot of work that would have taken you a long time to do by hand.  By saving the chi-squared test above, we can run diagnositcs by checking the observed, expected, and residuals $\text{res} = \frac{(O-E)^2}{E}$ for each combination of categories.

```{r eval=FALSE}
View(X2_sleep_vs_tv$observed)
View(X2_sleep_vs_tv$expected)
View(X2_sleep_vs_tv$residuals)
```
