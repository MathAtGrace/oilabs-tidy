...
If you haven't installed `statsr` on your computer yet, you might need to run
```{r eval=FALSE}
...
2.  The dataset spans a lot of years, but we are only interested in data from year 2016.
    Copy and paste the code below to filter the data `hfi` data frame for year 2016 and assign the result to a data frame named `hfi_2016`.
...
5.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of squares. Run the function several times. What was the smallest sum of squares that you got?
...
7.  If someone saw the least squares regression line and not the actual data, how would they predict a country's personal freedom score for one with a 3 rating for `pf_expression_control`? 
...
11. Choose another variable that you think would strongly correlate with `pf_score`.
    Produce a scatterplot of the two variables and fit a linear model.
    At a glance, does there seem to be a linear relationship?

12. How does this relationship compare to the relationship between `pf_score` and `pf_expression_control`?
    Use the $R^2$ values from the two model summaries to compare.
    Does your independent variable seem to predict `pf_score` better?
    Why or why not?

13. Check the model diagnostics using appropriate visualisations and evaluate if the model conditions have been met.
