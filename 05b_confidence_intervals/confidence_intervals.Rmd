...
In this lab, you'll start with a simple random sample of size 60 from the population.  Let's also set the randomization seed to the same number so that we all get the same random numbers.
...
2.  What percent of the adults in your sample think climate change affects their local community?  Remember to include your code of your computation. **Hint:** Just like we did with the population, we can calculate the proportion of those **in this sample** who think climate change affects their local community.

3.  If you were to create a second sample, would you expect that sample proportion to be identical to your first one?  Would you expect it to be similar? For both questions explain your reasoning.
...
1.  Copy the code above into your lab report. In the interpretation above, we used the phrase "95% confident". What does "95% confidence" mean?  Use the numbers output from your code in your sentence.
...
2.  If you were to take 20 similar samples and compute the confidence intervals, how many of them would you expect to not include the true proportion?
    ...
    df_ci <- eventReactive(input$go, {
      map_dfr(1:input$n_ci, store_ci,
        ...
        mutate(
          ...
          capture_p = factor(ifelse(x_lower < p() & x_upper > p(), "Yes", "No"), levels = c("No", "Yes"))
        )
    })
...
1.  Pick a different confidence level instead of 95%.  Would you expect confidence intervals with this new level to be wider or narrower than those 95% confidence intervals.

2.  Copy the code below into your lab report, change `0.95` to the confidence level you chose in the last problem, and run it.  Interpret your results.

```{r confidence interval-2 infer}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
...
3.  Using the app, calculate 50 confidence intervals at the confidence level you chose in the previous question, and calculate the proportion of intervals that include the true population proportion. How does this percentage compare to the confidence level selected for the intervals?
