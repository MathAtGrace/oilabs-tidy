...
1.  **Exercise** (for some reason I broke the formatting for exercises when I made changes to this lab -Dr. J) Add the plot from the code above into your lab report.
...
2.  **Exercise** Describe the distribution of responses in this sample using the words we used from section 2.1. How does it compare to the distribution of responses in the population.
...
3.  **Exercise** How does your `p_hat` compare to the true proportion?
...
4.  **Exercise** Would you expect the sample proportion to match the sample proportion of another student's sample? Why, or why not? If the answer is no, would you expect the proportions to be somewhat different or very different?

5. **Exercise** Take a second sample, also of size 50, and call it `samp2`. How does the sample proportion of `samp2` compare with that of `samp1`?

6. **Exercise** Suppose we took two more samples, one of size 100 and one of size 1000. Which would you think would provide a more accurate estimate of the population proportion?
...
7.  How many elements are there in `sample_props50`? Describe the sampling distribution, and be sure to specifically note its center. Make sure to include a plot of the distribution in your answer.
...
9.  Take a sample of size 15 from the population and calculate the proportion of people in this sample who think the work scientists do enhances their lives.
